  ...
  return (
    <div className="flex items-center justify-between border rounded p-3">
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => onUpdate(id, Math.max(0, item.quantity - 1))} className="px-2 py-1 bg-gray-100 rounded">-</button>
        <div className="px-3">{item.quantity}</div>
        <button onClick={() => onUpdate(id, item.quantity + 1)} className="px-2 py-1 bg-gray-100 rounded">+</button>
