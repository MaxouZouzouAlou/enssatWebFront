...
import { readSessionJson } from '../authBrowserState.js';
...
export default function RegisterForm({ onSwitchToLogin, preferredAccountType = null }) {
	...
	const [accountType, setAccountType] = useState(() => {
		const saved = readSessionJson(REGISTER_DRAFT_KEY, null);
		if (preferredAccountType) return preferredAccountType;
		return saved ? (saved.accountType || 'particulier') : 'particulier';
	});
	const [form, setForm] = useState(() => {
		const saved = readSessionJson(REGISTER_DRAFT_KEY, null);
		if (!saved) return emptyForm;
		const { accountType: _at, ...formData } = saved;
